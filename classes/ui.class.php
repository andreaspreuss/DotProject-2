...
class CTitleBlock_core {
	 ...
	 * Cells are added from left to right.
	 */
	function addBreadcrumb($link, $label, $icon='') {
		$link = str_replace('&amp;', '&', $link);
		$link = htmlspecialchars($link);
		$this->breadcrumbs[$link] = array($label, $icon);
	}
	/**
	...
	function show() {
		...
		if (count($this->breadcrumbs) || count($this->crumbs) || count($this->cells2)) {
			$crumbs = array();
			...
			if (count($this->breadcrumbs)){
				$s .= "\n\t" . '<td nowrap="nowrap">';
				$s .= "\n\t\t" . '<strong>' . implode(' >> ', $breadcrumbs) . '</strong>';
				$s .= "\n\t" . '</td>';
			}
			$s .= "\n\t" . '<td nowrap="nowrap">';
