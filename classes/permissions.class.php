      ...
      if ($module == 'projects') {                                                   
		...
		$macroproject_id = $item;
          ...
          // Grab $projet_id
  	      ...
  	      // Task owner
  	      $sql = ('SELECT task_owner FROM '.$dbprefix.'tasks WHERE task_id =' . $task_id);
  		    $task_owner = db_loadResult($sql);
          if ($task_owner == $GLOBALS['AppUI']->user_id) {$IsTaskOwner = true;}
          // Task participant
          $q = new DBQuery;
          $q->addTable('users', 'u');
          ...
          $sql = $q->prepare();
          $q->clear();
          $assigned_users = db_loadList($sql);
          foreach ($assigned_users as $val) {if ($val['user_id'] == $GLOBALS['AppUI']->user_id) {$IsTaskParticipant = true;}}
		  //we allow to user we can edit a project to edit all tasks of this project to
		  $q = new DBQuery;
          $q->addTable('dotpermissions', 'dp');
          $q->addTable('tasks', 't');
          $q->addQuery('dp.user_id, dp.permission');
          $q->addWhere('t.task_id = ' . $task_id);
          $q->addWhere('dp.axo = t.task_project');
		  $q->addWhere('dp.user_id = ' .$GLOBALS['AppUI']->user_id);
          $isAllowed = $q->loadList();
          $q->clear();
          foreach ($isAllowed as $allowToUser) {if ($allowToUser['permission'] == 'edit') {$IsProjectModifier = true;}}
      }      
      // Grab owner
      if (isset($project_id)) {
        $sql = ('SELECT project_owner FROM '.$dbprefix.'projects WHERE project_id =' . $project_id);
  	    $project_owner = db_loadResult($sql);
        if ($project_owner == $GLOBALS['AppUI']->user_id) {$IsProjectOwner = true;}
      }
	  if (isset($macroproject_id)) {
        $sql = ('SELECT macroproject_owner FROM '.$dbprefix.'macroprojects WHERE macroproject_id =' . $macroproject_id);
  	    $macroproject_owner = db_loadResult($sql);
        if ($macroproject_owner == $GLOBALS['AppUI']->user_id) {$IsMacroProjectOwner = true;}
      }
      // Rules
      if ($module == 'projects' && $IsProjectOwner) {$itemowner = true;}
	  if ($module == 'macroprojects' && $IsMacroProjectOwner) {$itemowner = true;}
      if ($module == 'tasks' && ($IsProjectOwner || $IsTaskOwner || $IsTaskParticipant || $IsProjectModifier)) {$itemowner = true;}
      if ($module == 'task_log' && ($IsProjectOwner || $IsTaskOwner || $IsTaskParticipant || $IsProjectModifier)) {$itemowner = true;}
    }
    // end of hack
		...
		if (!($result && $result['acl_id'])) {
			...
			//return $this->checkModule($module, $op, $userid);
			return ($itemowner ? $itemowner : $this->checkModule($module, $op, $userid));
		}
		dprint(__FILE__, __LINE__, 2, 
