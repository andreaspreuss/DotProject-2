...
<!--		<label for="ganttTaskFilter"><?php //echo $AppUI->_('Filter:')?></label>&nbsp;-->
		<select name="ganttTaskFilter" id="ganttTaskFilter" class="text" onchange="javascript:submitIt()" size="1">
			<?php 
				echo '<option value="0" '. (($ganttTaskFilter == '' OR $ganttTaskFilter == 0) ? ' selected="selected">' : '>') . '&lt;Show all tasks&gt; </option>';
				echo "\n";	
...
//	include 'gantt.php';
	...
	if ($addLinksToGantt == 1) {
		?>
		<iframe width="980px" height="500px" align="middle" src="<?php echo DP_BASE_URL . '/index.php' . $src; ?>" title="Please wait while the Gantt Chart is generated. (this might take up to a couple of minutes to complete)">Your current browser does not support frames. As a result this feature is not available.</iframe>
		<?php
	} else {
		?>
		<script language="javascript" type="text/javascript"> document.write('<img alt="Please wait while the Gantt chart is generated... (this might take a minute or two)" src="<?php echo $src; ?>" />') </script>
		<?php
	}
