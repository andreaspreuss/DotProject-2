...
//$q->addWhere($where_list . (($where_list) ? ' AND ' : '') . 't1.task_id = t1.task_parent'); //We need to compute it including all tasks
...
if($macroproject_id)//si on visualise un macroprojet
{
	$where = makeWhereClauseEachProjectOfAMacroProject($macroproject_id, 'task_project = ');
}
else//si on visualise autre chose(ici un projet)
{
...
?>
	<select name="add_users" multiple="multiple" class="multiselect"
...
<?php
		...
		if (is_array($p['tasks'])) {
			...
			<tr>
				...
				<a href="?m=projects&amp;a=view&amp;project_id=<?php echo $t1['task_project'];?>" class="hdr"><b><?php echo $AppUI->_('Project');?> : <?php echo $p['project_name']?></b></a></td>
