...
$config = array();
$config['mod_name'] = 'Monitoring and Control';
...
$config['mod_ui_name'] = 'Monitoring and Control';
$config['mod_ui_icon'] = 'graph-up.png';
$config['mod_description'] = "Monitoring and Control";
...
class CSetup_MonitoringAndControl{   
	...
	function install() {
		...
		$sql = "(
		  `meeting_item_id` INTEGER(11) NOT NULL AUTO_INCREMENT,
		  `meeting_item_description` TEXT COLLATE latin1_swedish_ci,
		...
		$q->addInsert('meeting_item_id', 1);
		$q->addInsert('meeting_item_description', 'Is the use and communication of data following the plan?');
		$q->exec();		
		
		$q = new DBQuery();
		$q-> addTable('monitoring_meeting_item');
		$q->addInsert('meeting_item_id', 2);
		$q->addInsert('meeting_item_description', 'Is the schedule being carried out according to plan?');
		$q->exec();		

		$q = new DBQuery();
		$q-> addTable('monitoring_meeting_item');
		$q->addInsert('meeting_item_id', 3);
		$q->addInsert('meeting_item_description', 'Is the stakeholder involvement following the plan?');
		$q->exec();		
		
		$q = new DBQuery();
		$q-> addTable('monitoring_meeting_item');
		$q->addInsert('meeting_item_id', 4);
		$q->addInsert('meeting_item_description', 'Were there changes in the risks?');
		$q->exec();		
		
		$q = new DBQuery();
		$q-> addTable('monitoring_meeting_item');
		$q->addInsert('meeting_item_id', 5);
		$q->addInsert('meeting_item_description', 'Were there risks?');
		$q->exec();		
		
		$q = new DBQuery();
		$q-> addTable('monitoring_meeting_item');
		$q->addInsert('meeting_item_id', 6);
		$q->addInsert('meeting_item_description', 'Are the costs being carried out according to plan?');
		$q->exec();								
