<?php /* PROJECTS $Id: projects.class.php,v 1.1.1.1 2006/08/16 06:50:21 aimass Exp $ */
/**
 *	@package dotProject
 *	@subpackage modules
...
class CProject extends CDpObject {
	...
	var $project_c_percent_complete = NULL;
	...
	// ensure changes of state in checkboxes is captured
        ...
        function load($oid=null , $strip = true) {
			...
			$q->clear();
			$q->addTable('projects');
			$q->addQuery('SUM(t2.task_actual_cost * 100) / 
                                        SUM(t2.task_total_cost) 
                                        AS project_c_percent_complete');
			$q->addJoin('tasks', 't1', 'projects.project_id = t1.task_project');
			// might need outer join.... pending to check model
			$q->addJoin('unitcost_task_costs', 't2', 't1.task_id = t2.task_id');
			$q->addWhere(" project_id = $oid");
                        $this->project_c_percent_complete = $q->loadResult();
