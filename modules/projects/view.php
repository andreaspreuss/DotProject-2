...
if ($hasTasks) {
    ...
    //$q->addWhere('t1.task_id = t1.task_parent'); //We need to compute it including all tasks
} else {
	$q->addQuery('com.company_name AS company_name, com_internal.company_name' 
				 . ' AS company_name_internal' 
...
$titleBlock->addBreadcrumb('?m=projects', 'Projects');
$titleBlock->addBreadcrumb('', $obj->project_name, 'applet3-48.png');
if ($canEdit) {
...
//update project_percent_complete in db of this project if change have been made 
smartUpdateProjectPercentComplete($project_id);
?>
...
function delIt() {
	...
	var wroteName = prompt("<?php 
	echo ($AppUI->_('confirmDelete') . ' \"' . $obj->project_name . '\" \n'. $AppUI->_('writeProjectName')
	. '\n' . $AppUI->_('canCopyPaste')); ?>", "<?php echo $AppUI->_('projectName')?>");
	if(projectName == wroteName)
	{
		document.frmDelete.submit();
	}
	else {
		alert("<?php echo $AppUI->_('incorrectProjectName', UI_OUTPUT_JS)?>");
	}
}
<?php } ?>
<?php
...
//some javascript functions may not appear on client side in case of user not having write permissions
//else users would be able to arbitrarily run 'bad' functions
//if ($canEdit) {  //All users can see Details of a project
?>
function doDetails() { 
...
echo $obj->project_color_identifier; ?>" colspan="2">
	<?php if ($canEdit) { ?><a href="?m=projects&amp;a=addedit&amp;project_id=<?php echo $project_id; ?>"> 
			<img src="images/icons/pencil.gif" title="<?php echo $AppUI->_("edit this project"); ?>"/>
		</a>
	<?php } ?>
	<a href="javascript:doDetails()">
